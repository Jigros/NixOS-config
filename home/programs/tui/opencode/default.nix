{pkgs, ...}: let
  restoreOpenCodeBackup = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      # Avoid copying a live SQLite database when OmniRoute is already running.
      ...
      # Some old backups kept these two files one directory higher.
      ...
      if [[ -f "$HOME/.omniroute/server.env" ]]; then
        chmod 600 "$HOME/.omniroute/server.env"
      fi
...
in {
  programs.opencode = {
    ...
    # Keep tools needed by coding agents available inside OpenCode's wrapped PATH.
